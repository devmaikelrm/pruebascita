#!/usr/bin/env python3
"""
Fix PM2 ecosystem con variables de entorno directas
"""
import requests
import subprocess
import sys
...
def main():
    print("\n" + "="*60)
    print("  Fix PM2 Ecosystem - Variables Directas")
    print("="*60 + "\n")
    ...
    print("[4/4] Corrigiendo PM2 ecosystem...")
    print("="*60 + "\n")
...
echo "▶ Creando nuevo ecosystem.config.cjs con variables directas..."
...
echo ""
echo "=== BOT LOGS ==="
pm2 logs bot --lines 30 --nostream 2>/dev/null || true

echo ""
echo "=== WORKER LOGS ==="
pm2 logs worker --lines 30 --nostream 2>/dev/null || true
...
""", get_pty=True)
    ...
    for i in range(1, 4):
        try:
            ...
            stdin, stdout, stderr = ssh2.exec_command("pm2 status && echo '' && echo '=== BOT LOGS ===' && pm2 logs bot --lines 40 --nostream && echo '' && echo '=== WORKER LOGS ===' && pm2 logs worker --lines 40 --nostream", get_pty=True)
            while True:
                line = stdout.readline()
                if not line:
                    break
